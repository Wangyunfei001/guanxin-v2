...
| 存储 | 数据 | 隔离方式 |
|---|---|---|
| SQLite | documents、chunks、conversations、messages、agent_configs、tool_approvals、schema_version | tenant_id；会话再校验 user_id |
...
自动测试 mock LLM/Embedding，不调用真实付费模型。生产部署、审计日志、限流、远程 MCP SSE 和用户账户 SQLite 迁移不在当前版本范围。
