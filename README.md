...
研究结果在会话中展示“主张—原文摘录—来源链接”。摘录必须匹配实际读取的正文，保存获取时间、内容哈希和最终 URL；所有条目仍为待人工审查，文字匹配不等于原文支持整个主张。原文首次获取后保持不变，当前不提供刷新或自动语义批准。

首次搜索固定会话累计预算：自动/快速模式最多 2 次请求，深度模式最多 4 次；取消、继续、重启或切换模式不重置。搜索串行准入，失败用量未知时暂停后续搜索。服务商报告的动作阈值为 20/40，只阻止后续请求，无法截断当前请求内部动作；这不是账户级费用上限。

`RESEARCH_SOURCE_HOSTS` 配置可读取原文的可信 HTTPS 域名，默认 `docs.langchain.com,reference.langchain.com,api-docs.deepseek.com`。仅接受本会话搜索结果中的 URL，逐次检查重定向与公共地址，正文最多 1 MB，总读取期限 25 秒；搜索到其他域名仍可展示链接，但不能登记为已匹配原文的证据。域名须由管理员可信配置，不能把此读取器当作允许任意 URL 的通用代理。

真实复测及限制见 [引用与预算评估](docs/evaluations/2026-09-09-citation-budget-assessment.md)。
